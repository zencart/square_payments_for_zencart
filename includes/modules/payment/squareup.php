...
class squareup extends base
{
    ...
    function confirmation()
    {
        $confirmation = [
            'fields' => [
                [
                    'title' => MODULE_PAYMENT_SQUAREUP_TEXT_CREDIT_CARD_TYPE,
    ...
    function before_process()
    {
        ...
        if (MODULE_PAYMENT_SQUAREUP_TESTING_MODE == 'Live') {
...
// SIMPLIFIED OAUTH TOKENIZATION
    ...
    function disableDueToInvalidAccessToken()
    {
        ...
        zen_mail(STORE_OWNER_EMAIL_ADDRESS, STORE_OWNER_EMAIL_ADDRESS, 'SquareUp Payment Module Problem: Critical', $msg, STORE_NAME, EMAIL_FROM, ['EMAIL_MESSAGE_HTML' => $msg], 'payment_module_error');
        trigger_error('Square Payment Module token expired' . (MODULE_PAYMENT_SQUAREUP_REFRESH_EXPIRES_AT != '' ? ' on ' . MODULE_PAYMENT_SQUAREUP_REFRESH_EXPIRES_AT : '') . '. Payment module has been disabled. Please login to Admin and re-authorize the module.',
            E_USER_ERROR);
    }
    ...
    function getAuthorizeURL()
    {
        ...
        $params = http_build_query(
            [
                'client_id' => MODULE_PAYMENT_SQUAREUP_APPLICATION_ID,
